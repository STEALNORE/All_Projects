                                        ...
                                        float FrameDeltaTime, const FAnimNotifyEventReference& EventReference)
{
	...
	{
		//If Attack can be triggered multiple times, then use the exposed variable since the private gets set to false when it is true, that means it will continue to run the multiple hit logic after attacking once
		DoCollisionCheck(MeshComp);
	}
...
void UANS_AIAttackCollision::DoCollisionCheck(USkeletalMeshComponent* MeshComp)
{
	...
	bool bHit = UKismetSystemLibrary::SphereTraceSingle(MeshComp->GetWorld(), InSocket, OutSocket, Radius, TraceChannel , true,  ActorsToIgnore, DoTrace,
															HitResult, true );
	...
	if(bHit)
	{
		if(HitActor && HitResult.GetActor()->GetComponentByClass(UAC_FFCC_Base::StaticClass()))
		{
			...
			if(!(FFCS->GetTag() ==HitCollisionExlusionTag) || !(FFCS->GetTag() == FinisherTag))
			{
