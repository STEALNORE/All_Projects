...
; Возврат: RAX - 1 / 0 : true / false
	...
	; Если EAX == 0, то вернём 1, иначе - 0
	cmp eax, 0
	cmove eax, edx
	cmovne eax, ebx
...
; Возврат: RAX - 1 / 0 : true / false
	...
	test ebx, 100001000b  ; Проверяем наличие BMI1 и BMI2
	jz _0

	inc rax  ; RAX = 1, если есть
...
;-------------------------------------------------------------------------------------------------------------
Test_Command proc
; extern "C" void Test_Command(char *left_singles, char *right_singles, short *result_singles);
; Параметры:
; RCX - left_values
; RDX - right_values
; R8 - result
